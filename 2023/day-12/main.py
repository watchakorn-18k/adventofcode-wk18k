"""
https://github.com/LuisGC/advent-of-code/blob/main/2023/day-12/main.py
"""
...
def count_permutations(pattern: str, splits: List[int]) -> int:
    ...
    for before in range(len(pattern) - after - current_split + 1):
        ...
        if all(char_1 == '?' or char_1 == char_2 for char_1, char_2 in zip(pattern, candidate)):
            count += count_permutations(pattern[len(candidate):],
                                        remaining_splits)
    return count
...
with open("input", encoding="utf-8") as f:
    ...
    print("Part 2: The sum of all options after unfolding is ",
          sum_all_permutations(rows, unfolding=True))
